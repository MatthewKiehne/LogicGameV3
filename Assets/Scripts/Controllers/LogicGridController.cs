...
public class LogicGridController : MonoBehaviour
{
    ...
    private void display()
    {
        ...
        foreach (LogicComponent comp in logicGrid.GetLogicComponents())
        {
            ...
            logicCube.transform.localPosition = logicPosition * renderScale;
            logicCube.transform.localScale = new Vector3(comp.GetBody().GetArea().width, comp.GetBody().GetArea().height, 1 * zScale) * renderScale;
        ...
        foreach (LogicConnection connection in logicGrid.GetLogicConnections())
        {
            ...
            go.transform.localPosition = this.vector2IntToVector3(connection.GetBody().GetArea().position) * renderScale;
            ...
            go.transform.localPosition += new Vector3((connection.GetBody().GetArea().width - 1) / 2f, (connection.GetBody().GetArea().height - 1) / 2f, 0) * renderScale;
